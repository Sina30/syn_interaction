...
function SetupCameraPrompt()
    Citizen.CreateThread(function()
        cameraprompt=false
        CameraPickup=nil
        local str = 'Nehmen Sie die Kamera herunter'
...
AddEventHandler('prop:cigarettes', function() 
    FPrompt("Rauchen beenden", 0x3B24C470, false)
    LMPrompt("Inhalieren", 0x07B8BEAF, false)
    EPrompt("Haltung ändern", 0xD51B784F, false)
    ExecuteCommand('close')
    local ped = PlayerPedId()
    local male = IsPedMale(ped)
    local x,y,z = table.unpack(GetEntityCoords(ped, true))
...
AddEventHandler('prop:cigar', function()
    ...
    --PlaySoundFrontend("Core_Full", "Consumption_Sounds", true, 0)
    ExecuteCommand('close')
    FPrompt('Rauchen aufhören', 0x3B24C470, false)
...
AddEventHandler('prop:syn', function() 
    FPrompt("Weglegen", 0x3B24C470, false)
    LMPrompt("Benutzen", 0x07B8BEAF, false)
...
AddEventHandler('prop:fan', function() 
    FPrompt("Weglegen", 0x3B24C470, false)
    LMPrompt("Kleine Welle", 0x07B8BEAF, false)
    ExecuteCommand('close')
    local ped = PlayerPedId()
    local male = IsPedMale(ped)
    local x,y,z = table.unpack(GetEntityCoords(ped, true))
...
AddEventHandler('prop:chewingtobacco', function()
    ...
    --PlaySoundFrontend("Core_Full", "Consumption_Sounds", true, 0)
    FPrompt("Ziel", 0x3B24C470, false)
    LMPrompt("Etwas tun", 0x07B8BEAF, false)
    EPrompt("Haltung ändern", 0xD51B784F, false)
    ExecuteCommand('close')
    local ped = PlayerPedId()
    local x,y,z = table.unpack(GetEntityCoords(ped, true))
